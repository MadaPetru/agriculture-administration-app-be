...
WHERE type = 'PK'
  AND OBJECT_NAME(parent_object_id) = 'farming_land_statistics_per_year';
...
WHERE type IN ('PK', 'UQ', 'FK') -- Primary Key, Unique Constraint, Foreign Key
  AND parent_object_id = OBJECT_ID('farming_land_statistics_per_year');
...
BEGIN
        SET
@SQL = 'ALTER TABLE farming_land_statistics_per_year DROP CONSTRAINT ' + QUOTENAME(@ConstraintName);
